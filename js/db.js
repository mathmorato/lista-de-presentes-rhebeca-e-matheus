  ...
  async init(onDataChange) {
    ...
    if (window.supabase && supabaseUrl && supabaseKey) {
      try {
        ...
        console.log('[WeddingDB v.1.0.5] Supabase client inicializado:', supabaseUrl);
        ...
        this.syncFromSupabase().catch(err => {
          console.warn('[WeddingDB v.1.0.5] Sincronização em background inicial (IndexedDB ativo):', err);
        });
      } catch (err) {
        console.warn('[WeddingDB v.1.0.5] Falha ao inicializar Supabase. Operando modo IndexedDB offline:', err);
      }
    }
